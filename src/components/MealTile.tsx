 ...
 */
const SIZES = {
  chip: { box: 'h-7 w-7 rounded-lg md:h-6 md:w-6 md:rounded-[7px]', icon: 'h-4 w-4 md:h-3.5 md:w-3.5', badge: false },
  row: { box: 'h-8 w-8 rounded-lg', icon: 'h-4.5 w-4.5', badge: false },
  card: { box: 'h-12 w-12 rounded-xl md:h-10 md:w-10 md:rounded-[11px]', icon: 'h-6 w-6 md:h-5 md:w-5', badge: true },
 ...
 */
export default function MealTile({ category, size, surface = 'surface-1', leftovers = false, className = '' }: MealTileProps) {
  ...
  return (
    <span
      aria-hidden
      className={`relative flex flex-shrink-0 items-center justify-center ${box} ${tileClasses(category?.main)} ${className}`}
    >
      ...
      {secondary && (
        ...
        >
          <Icon name={secondary} className="h-[72%] w-[72%]" strokeWidth={2.2} />
        </span>
      )}
      {badge && leftovers && (
        <span
        ...
        >
          <Icon name="leftovers" className="h-[70%] w-[70%]" strokeWidth={2.2} />
        </span>
      )}
    </span>
  )

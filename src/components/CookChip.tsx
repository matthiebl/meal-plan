...
type CookChipProps = {
  ...
  onTodayRow: boolean
  /** The card's second line: how long since the meal was last cooked, or where leftovers came from. */
  detail: string
  canMoveLeft: boolean
...
/**
 * One cook or leftovers, as a card: the meal's tile, name, and a detail line
 * beneath — the same size at every width, full-width on a phone and a fixed
 * width side by side from `md` up. Leftovers styling is derived from `kind`,
 * never chosen: an outline instead of a fill, a dimmed tile, and a
 * return-arrow as the tile's badge. See PLAN.md §5.
 *
 ...
 * the leftovers gesture; clicking it adds leftovers to the next day, the
...
export default function CookChip({
  ...
  return (
    <div
      ...
      className={`relative flex w-full items-center gap-1 rounded-2xl p-2 md:w-92 ${
        menuOpen ? 'z-40' : ''
      } ${
        isLeftovers
    ...
    >
      <button
        ...
        className="flex h-full min-w-0 flex-1 cursor-grab touch-pan-y items-center gap-3 pr-1 text-left active:cursor-grabbing"
      >
        <MealTile
      ...
      {!isLeftovers && (
        <button
          ...
          className="flex h-9 w-9 shrink-0 touch-none items-center justify-center rounded-[11px] text-ink-3 transition-colors hover:bg-line hover:text-ink"
        >
          <Icon name="leftovers" className="h-5 w-5" />
        </button>
      )}
      ...
      <Popover
        ...
        panelClassName="pointer-events-auto w-[20rem]"
        anchorRef={chipRef}
